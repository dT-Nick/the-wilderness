...
export function updateMapOneZeroState(
  changes:
    | Partial<typeof mapOneZeroState>
    | ((state: typeof mapOneZeroState) => Partial<typeof mapOneZeroState>)
) {
  if (typeof changes === 'function') {
    Object.assign(mapOneZeroState, changes(mapOneZeroState))
  } else {
    Object.assign(mapOneZeroState, changes)
  }
}

export function drawMapOneZero() {
  const { map } = mapOneZeroState
  drawBackgroundFromMap(map)
}

export function handleMapOneZeroInput() {
  const { map } = mapOneZeroState
  const restrictedCoords = deriveRestrictedCoordsFromMap(map)

  handlePlayerMovement(restrictedCoords)
  handleMapOneZeroExit()
}

export function handleMapOneZeroExit() {
